...
function active(req, res){
	...
	connection.query(queryString, function(err, rows, fields){
		if(err){		
			...
			if(rows.length == 0){
				res.send(SYSTEM_ERROR);//Không có mã code ứng với id, người dùng tự kích hoạt lấy lại code
			}else{
				if(code == rows[0].code){
...
io.sockets.on("connection", function(socket){	
							...
							response.on("end", function(){
								...
								parseString(text, function (err, result) {
									if(err){
										...
										var tuVi = null;
										if(result.tamlinh.tuvi != undefined){
	...
	socket.on("newfriendchat",function(data){
		...
		var queryString = "select id,avatar,username,sex from user where id='"+data+"'";
		var emittoclient = function(rows){
			//console.log(rows[0]);
			socket.emit("newfriendchat",rows[0]);
		};
		queryFun(queryString,null,emittoclient,null);
