...
io.sockets.on("connection", function(socket){	
	...
	socket.on("newfriendchat", function(id){
		var queryString = "select avatar, username, sex from user where id='"+id+"'";
		connection.query(queryString, function(err, rows, fields){
			if(err){
				socket.emit("newfriendchat", {"status":FAILD, "id":id});
			}else{
				if(rows.length != 0){
					socket.emit("newfriendchat", {"status":SUCCESS, "id":id, "username":rows[0].username, "avatar":rows[0].avatar, "sex":rows[0].sex});
				}else{
					socket.emit("newfriendchat", {"status":FAILD, "id":id});
				}
			}
			...
			connection.query(queryString, function(err, results){
				if(!err){
					if((map.search(data.id)!=null)||(mapOffline.search(data.id)!=null)){
						socket.broadcast.to(data.id).emit("nhanyeucauketbanmoi", {   
																			  "id":uID,
																			  "avatar":data.avatar,
	...
	socket.on("huyguiyeucauketban", function(id){
		...
		if(uID != undefined){
			var queryString = "delete from requestfriend where (sender='"+uID+"' and receiver='"+id+"') or (sender='"+id+"' and receiver='"+uID+"')";
			connection.query(queryString, function(err, results){
				if(!err){
	...
	socket.on("tuchoiketban", function(id){
		...
		if(uID != undefined){
			var queryString = "delete from requestfriend where (sender='"+id+"' and receiver='"+uID+"') or (sender='"+uID+"' and receiver='"+id+"')";
			connection.query(queryString, function(err, results){
				if(!err){
	...
	//Bàn luận more về trả về trạng thái, xóa notification
	socket.on("huybanbe", function(id){
		...
		if(uID != undefined){
			...
			connection.query(queryString, function(err, results){
				if(!err){
					//socket.emit("ketquahuyketban", {"status":SUCCESS, "id":id});
					var isOnline2 = true;
					...
					//socket.emit("ketquahuyketban", {"status":FAILD, "id":id});
				}
	...
	socket.on("chapnhanketban", function(data){
		...
		if(uID != undefined){
			...
			connection.query(queryString, function(err, results){
				if(err){	
					socket.emit("ketquaxacnhanketban", {"status":FAILD, "id":data.id});
				}else{	
					var isOnline2 = true;
					var sID = map.search(data.id);
					if(sID == null){
						sID = mapOffline.search(data.id);
						isOnline2 = false;
					}
					if(sID != null){
						socket.emit("ketquaxacnhanketban", {"status":SUCCESS, "id":data.id, "isOnline":isOnline1?isOnline2:false});
						console.log("HAHAHA "+data.avatar+" "+data.status+" "+data.avatar);
						socket.broadcast.to(data.id).emit("chapnhanketban", { 
																			  "id":uID,	
																			  "avatar":data.avatar,
																			  "username":data.username,
																			  "status":data.status,
																			  "isOnline":isOnline2?isOnline1:false
																		    });
						if(isOnline1&&isOnline2){
							socket.join(sID);
							allSockets[sID].join(socket.id);
						...
						console.log("HAHAHA "+data.avatar+" "+data.status+" "+data.avatar);
						socket.emit("ketquaxacnhanketban", {"status":SUCCESS, "id":data.id, "isOnline":false});
						queryString = "insert into notification(id1, id2, code, created_date) values('"+data.id+"','"+uID+"','0','"+dateTime+"')";
						connection.query(queryString, function(err, results){
						});
					}
					queryString = "delete from requestfriend where (sender='"+data.id+"' and receiver='"+uID+"') or (sender='"+uID+"' and receiver='"+data.id+"')";
					connection.query(queryString, function(err, results){
					});					
				}
			});
		}else{
			socket.emit("ketquachapnhanketban", {"status":FAILD, "id":data.id});
		}
	});
	...
	socket.on("dohoptuvi", function(data){
		...
		console.log(nam+" "+nu);
		//var namSinhNam = getLunarYearFromSolar(parseInt(nam[0]), parseInt(nam[1]), parseInt(nam[2]), 7.0);
		//var namSinhNu = getLunarYearFromSolar(parseInt(nu[0]), parseInt(nu[1]), parseInt(nu[2]), 7.0);
		console.log(nam+" "+nu);
		
		var options = {
				host: 'api.xalovansu.com',
				port: 80,
				path: "/tuVi/chamDiemCapDoi/xem/namsinhnam/"+nam[0]+"/namsinhnu/"+nu[1]+"/ngaysinh1/"+nam[0]+"/thangsinh1/"+nam[1]+"/ngaysinh2/"+nu[0]+"/thangsinh2/"+nu[1]+"/key/tviapp@)!@",
	...
	var queryString = format("Select * from delay_message where ID_channel = '{0}'  and Receiver = '{1}' and Sent = 0 ",rows[index].ID_channel,data);	
	...
	var sendHistoryMessage = function(){
		var queryString = format("SELECT ID_channel,Max(Number) as Number FROM  delay_message where Sender = '{0}' or Receiver = '{0}'  GROUP BY ID_channel",data);
	...
	var sendNewMessage = function(){
		...
		var queryString = format("Select * from delay_message where Receiver = '{0}' and Sent = 0 ",data);
	...
	var connectInfo = function(rows){
		  ...
		  console.log("Co client ket noi ten "+rows[0].username+ " id: "+data);
