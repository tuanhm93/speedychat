...
io.sockets.on("connection", function(socket){	
	...
	socket.on("timkiemuser", function(data){
		...
		var nam = data.dateOfBirth.split("-");
		
		var queryString = "select * from user where username like '%"+userName+"%'";
		connection.query(queryString, function(err, rows, fields){
			if(err){
	...
	var sendHistoryMessage = function(){
		var queryString = format("Select * from delay_message where Sender = '{0}' or Receiver = '{0}'  group by ID_channel",socket.id);
	...
	var sendNewMessage = function(){
		...
		var queryString = format("Select * from delay_message where Receiver = '{0}' and Sent = 0 ",socket.id);
...
socket.on('daNhanMotPhanHinhAnh',function(data){
  ...
  socket.on('typing_message',function(data){
   ...
   var date = new Date();
    ...
    Date.masks.default = 'hh:mm:ss';
    time=time.format() ;
         console.log('typing_message'+data+" "+socket.id);
        if(data>0){                //Nếu là tin nhắn từ chat 2 người 
          var idU = idChanelToidUser(data);   //Chuyển từ ID Chanel sang 2 id người nhận và người gửi
          console.log(idU);
          if(idU.id1 == socket.id){
