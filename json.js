		...
		if(uID != undefined){
			...
			connection.query(queryString, function(err, results){
				if(err){	
					...
					socket.emit("trangthaiketban", {"status":SUCCESS, "id":data.id, "isOnline":isOnline1?isOnline2:false});					
				}
			});
		}
	});
				...
				parseString(text, function (err, result) {
					if(err){
						var doHopTuVi = {
										 ...
										 "can":2,
										 "menh":4,
										 "ketLuan":"Tổng điểm: 10/10. Hệ thống đang gặp sự cố vui lòng thử lại sau!"
										};
	...
	socket.on("tuvihangngay", function(data){
		...
		var requestSend = http1.request(options, function(response){
				...
				parseString(text, function (err, result) {
					if(err){
						var ngayGio = {  "thu": "Thứ 6",
										 ...
										 "tenThang":"Tên Tháng",
										 "tenNam":"Tên Năm",
										 "gioHoangDao": "Giờ Hoàng Đạo",
										 "gioHacDao": "Giờ Hắc Đạo"
									  };
						socket.emit("tuvihangngay", {"thongTinChung":"Hệ thống đang gặp sự cố vui lòng thử lại!",
													 "sucKhoe":"Chỉ số: 100%.",
													 "tinhCam":"Chỉ số: 100%.",
													 "triTue":"Chỉ số: 100%.",
													 "trucGiac":"Chỉ số: 100%.",
													 "yNghia":"Hệ thống đang gặp sự cố vui lòng thử lại!",
													 "ngayGio": ngayGio});
					}else{								
						var item = result.tamlinh.tuvi[0].root[0].data[0].item;
						var tuViHangNgay = {
			...
			response.on("error", function(err){
				...
				socket.emit("tuvihangngay", {
											"thongTinChung":"Hệ thống đang gặp sự cố vui lòng thử lại!",
											"sucKhoe":"Chỉ số: 100%.",
											"tinhCam":"Chỉ số: 100%.",
											"triTue":"Chỉ số: 100%.",
											"trucGiac":"Chỉ số: 100%.",
