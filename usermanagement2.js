...
io.sockets.on("connection", function(socket){	
				...
				if(rows.length == 0){
					...
					if(rows[0].password == data.password){
						...
						if(oldSocketId != null){
							...
							socket.broadcast.to(oldSocketId).emit("friend offline", { "id":rows[0].id, "sID":oldSocketId});
	...
	socket.on("setting offline", function(data){
		console.log("setting offline: "+data.id);
		map.remove(socket.id);
		mapOffline.set(socket.id, data.id);
		var fri = data.fri;
		var length = fri.length;
		//Gửi thông tin offline tới bạn bè đang online						
		for(var i=0;i<length;i++){
			var friID = fri[i].id;
			var sID = map.search(friID);
			if(sID != null){
				console.log("Gui thong tin offline toi thang: "+data.id);
				socket.broadcast.to(friID).emit("friend offline", {"id":data.id, "sID":socket.id});
				socket.emit("friend offline", {"id":friID, "sID":sID}); 
			}
		}
	});
	
	socket.on("setting online", function(data){
		console.log("setting online: "+data.id);
		map.set(socket.id, data.id);
		...
		for(var i=0;i<length;i++){
			var friID = fri[i].id;
			var sID = map.search(friID);
			console.log(sID+" "+friID);
			if(sID != null){
				console.log("Gui thong tin online toi thang: "+data.id);
				socket.broadcast.to(friID).emit("friend online", {"id":data.id, "sID":socket.id});
				socket.emit("friend online", {"id":friID, "sID":sID}); 
			}
		}
	});
		...
		if(uID != undefined){
			...
			connection.query(queryString, function(err, rows, fields){
				if(err){
					...
					socket.emit("nhanthongbao", {"status":SUCCESS,
												  "notifies":notifies
												});
				}
			});
	...
	socket.on("xemthongbao", function(){
		var uID = map.get(socket.id);
		if( uID == undefined){
			uID = mapOffline.get(socket.id);
		}
	...
	socket.on("guiyeucauketban", function(data){
		var uID = map.get(socket.id);
		if( uID == undefined){
			uID = mapOffline.get(socket.id);
		}
		if(uID != undefined){
			...
			dateTime = dateTime.getDate()+"-"+(dateTime.getMonth()+1)+"-"+dateTime.getFullYear()+" "+
				dateTime.getHours()+":"+dateTime.getMinutes()+":"+dateTime.getSeconds();
			var queryString = "insert into requestfriend(sender, receiver, created_date) values('"+uID+"','"+data.id+"','"+dateTime+"')";
			connection.query(queryString, function(err, results){
				if(!err){
					if((map.search(data.id)!=null)||(mapOffline.search(data.id)!=null)){
						socket.broadcast.to(data.id).emit("yeucauketban", {   "id":uID,
																			  "avatar":data.avatar,
																			  "username":data.username,
																			  "status":data.status
																		   });
					}else{
						queryString = "insert into notification(id1, id2, code, created_date) values('"+data.id+"','"+uID+"','1','"+dateTime+"')";
						connection.query(queryString, function(err, results){
						});
					}	
				}
			...
			connection.query(queryString, function(err, results){
				if(!err){
					...
					if((map.search(id)!=null)||(mapOffline.search(id)!=null)){
						socket.broadcast.to(id).emit("huyguiyeucauketban", {"id":id});
					}
				}
			});
		}
	...
	socket.on("huyketban", function(id){
		var uID = map.get(socket.id);
		if( uID == undefined){
			uID = mapOffline.get(socket.id);
		}
		if(uID != undefined){
			var queryString = "delete from listfriend where sender='"+id+"' and receiver='"+uID+"'";
			connection.query(queryString, function(err, results){
				queryString = "delete from listfriend where sender='"+uID+"' and receiver='"+id+"'";
				connection.query(queryString, function(err, results){
					queryString = "delete from requestfriend where sender='"+id+"' and receiver='"+uID+"'";
					connection.query(queryString, function(err, results){
						queryString = "delete from requestfriend where sender='"+uID+"' and receiver='"+id+"'";
						connection.query(queryString, function(err, results){
						});
					});
				});
				if(!err){
					if((map.search(id)!=null)||(mapOffline.search(id)!=null)){
						socket.broadcast.to(id).emit("huyketban", {"id":uID});
					}
				}
			});
		}
	...
	socket.on("chapnhanketban", function(data){
		...
		if(uID != undefined){
			...
			dateTime = dateTime.getDate()+"-"+(dateTime.getMonth()+1)+"-"+dateTime.getFullYear()+" "+
					dateTime.getHours()+":"+dateTime.getMinutes()+":"+dateTime.getSeconds();
			queryString = "insert into listfriend(sender, receiver, created_date) values('"+data.id+"','"+uID+"','"+dateTime+"')";
			connection.query(queryString, function(err, results){
				if(err){	
					socket.emit("trangthaiketban", {"status":FAILD, "id":data.id});
				}else{
					socket.emit("trangthaiketban", {"status":SUCCESS, "id":data.id});
					if((map.search(data.id)!=null)||(mapOffline.search(data.id)!=null)){
						socket.broadcast.to(data.id).emit("chapnhanketban", { "id":uID,	
																			  "avatar":data.avatar,
																			  "username":data.username			  
																		   });
					}else{
						queryString = "insert into notification(id1, id2, code, created_date) values('"+data.id+"','"+uID+"','0','"+created_date+"')";
						connection.query(queryString, function(err, results){
						});
					}		
				}
	...
	/*socket.on("reconnect", function(data){
		var oldSocketId = map.search(data.id);
		if(oldSocketId != null){
			socket.broadcast.to(oldSocketId).emit("friend offline", { "id":data.id, "sID":oldSocketId});
			map.remove(oldSocketId);
		}else{
			oldSocketId = mapOffline.search(data.id);
			if(oldSocketId != null){
				mapOffline.remove(oldSocketId);
			}
		}
		...
		queryString = "SELECT id, avatar, username, status FROM user WHERE id in (SELECT sender FROM listfriend WHERE receiver='"
							+data.id+"' UNION SELECT receiver FROM listfriend WHERE sender='"+data.id+"')";
		connection.query(queryString, function(err, rows, fields){
			var fri = [];	
			if(!err){	
				...
				queryString = "SELECT id, avatar, username, status FROM user WHERE id in (SELECT sender FROM requestfriend WHERE receiver='"
								+data.id+"' UNION SELECT receiver FROM requestfriend WHERE sender='"+data.id+"')";
				connection.query(queryString, function(err, rows, fields){
					var friReq = [];	
					if(!err){	
						for(var i=0;i<rows.length;i++){
						...
						if((fri.length !=0) && (friReq.length != 0)){
							for(i=0;i<fri.length;i++){
								for(j=0;j<friReq.length;j++){
									if(fri[i].id == friReq[j].id){
										fri.splice(i,1);
										i--;
										break;
									}
		...
		if(data.isOnline == 1){
			map.set(socket.id, data.id);//Ánh xạ id socket sang id user
	...
	socket.on("disconnect", function(){
		...
		if(uID != undefined){
			...
			socket.broadcast.to(socket.id).emit("friend offline", { "id":uID, "sID":socket.id});
		}else{
				...
				parseString(data, function (err, result) {
					if(err){
						...
						var doHopTuVi = {
										...
										"cung": parseInt(item[1].value[0].charAt(item[1].value[0].length-4)),
										"can": parseInt(item[2].value[0].charAt(item[2].value[0].length-4)),
										"chi": parseInt(item[3].value[0].charAt(item[3].value[0].length-4)),
										"menh": parseInt(item[4].value[0].charAt(item[4].value[0].length-4)),
	...
	var connectInfo = function(rows){
		...
		console.log("Co client ket noi ten "+rows[0].username);
	
		  id = rows[0].id;
          socket.nickname = rows[0].username;
		  socket.idU = rows[0].id;
          users[id]= socket;
          nametoid[socket.nickname]=id;
          nicknames[id]=socket.nickname;
	...
	if(user.active == 0){			
		...
		connection.query(queryString, function(err, rows, fields){
			...
			if(!err){	
				...
				console.log("HAHAHAHA co:"+fri.length);
				//Lấy danh sách yêu cầu kết bạn
				queryString = "SELECT user.id, user.avatar, user.username, user.status FROM user, requestfriend WHERE user.id = requestfriend.sender and requestfriend.receiver='"+user.id+"'";
				connection.query(queryString, function(err, rows, fields){
					var friReq = [];	
					if(!err){	
						for(var i=0;i<rows.length;i++){
						...
						if(!data.isDisableOnline){
							//Gửi thông tin online tới bạn bè đang online						
							for(var i=0;i<fri.length;i++){
								console.log("Gui thong tin online toi client");
								var friID = fri[i].id;
								var sID = map.search(friID);
								console.log(sID+" "+friID);
								if(sID != null){
									console.log("Gui online toi thang: ", user.id);
									socket.broadcast.to(friID).emit("friend online", {"id":user.id, "sID":socket.id});
									socket.emit("friend online", {"id":friID, "sID":sID}); 
								}
							}
						}
